...
This split is what makes the suite safe to run repeatedly and trivial to undo — `/agentic-test-clean` can delete every trace because agent tests are unmistakable by name.
...
- **Mutation testing** (optional but recommended) — coverage says a line was *executed*; a mutation score says a change to it would be *caught*. `/agentic-mutation-check` reports and closes the gap.
