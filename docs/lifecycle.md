...
## Stage 1 — Generate (`/agentic-unit-test`)
...
> 💡 One file at a time (`/agentic-unit-test src/thing.ts branch 90 per-file`) gives faster feedback and cheaper resumes than a whole-project run.

## Stage 2 — Verify (`/agentic-mutation-check`)
...
## Stage 3 — Refactor (`/agentic-refactor`)
...
## Stage 4 — Re-lock (`/agentic-test-update`)
...
- `/agentic-coverage-report` — standings check at any point, touches nothing.
- `/agentic-test-clean` — full teardown; the filename convention guarantees it removes exactly and only what the suite created.
